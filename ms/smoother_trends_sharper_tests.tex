...
Our contribution is to formalize this connection, implement the Rauch--Tung--Striebel (RTS) smoother for event study coefficients, and demonstrate that it substantially outperforms not only raw estimates but also the natural competing approaches---parametric trend models, James--Stein shrinkage, empirical Bayes, and Savitzky--Golay---for estimation, with modest gains for parallel trends testing. The key advantage is that the Kalman smoother uses the \emph{known heteroskedastic standard errors} to adapt its smoothing intensity period by period, while simultaneously estimating the treatment effect level and its rate of change.
...
\begin{remark}[Smoothness Governs Estimation, Not Identification]
The smoothness assumption is a regularization prior for \emph{estimation}, not an identifying assumption. Identification of period-specific treatment effects still rests entirely on the parallel trends assumption. The bootstrap-calibrated pre-trend test draws from $\hat{\beta}^{(b)}_t \sim N(0, \hat{\sigma}^2_t)$ under $H_0$, not from the state-space model. Smoothing improves MSE under correct specification of Assumption~\ref{a:smooth}, but the causal interpretation derives from the research design.
\end{remark}
...
\begin{remark}[Linear vs.\ Global Optimality]
The Kalman smoother is the minimum MSE \emph{linear} estimator under the state-space model, regardless of the error distribution, requiring only finite second moments. It is also the global minimum MSE estimator (among all estimators, linear or nonlinear) if and only if all noises are Gaussian. With non-Gaussian errors (e.g., heavy tails), nonlinear estimators such as particle filters could in principle achieve lower MSE, though the practical gains in event study applications are likely small given CLT-based normality of $\hat{\beta}_t$ when the number of clusters is moderate.
\end{remark}

\begin{remark}[Estimand Unchanged]
The smoothed estimate $\hat{\beta}^{\rm KS}_t$ targets the same period-specific average treatment effect as the raw $\hat{\beta}_t$. Smoothing is a shrinkage estimator that improves MSE under the smoothness prior without changing the estimand. The causal interpretation is unchanged: $\beta_t$ remains the ATT in period $t$.
\end{remark}
...
\end{equation}
This test is sensitive to trending violations---precisely the anticipation effects most concerning for identification. However, the derivative test has zero power against pre-period level shifts: a constant has zero derivative, so discrete jumps in the pre-period are invisible to this test. When anticipation takes the form of a level shift rather than a trending violation, researchers should combine the derivative test with a level-based test such as the Kalman Wald or raw Wald.
...
The bootstrap-calibrated test has rejection probability $\alpha + O(B^{-1/2})$ under $H_0$ and Assumptions~\ref{a:normal}--\ref{a:known_se}. This follows from the parametric bootstrap being exact when the null DGP is correctly specified \citep{efron1994introduction}.
\end{proposition}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../figs/paper_fig1.pdf}
...
First, the Kalman smoother has the lowest level MSE across all non-trivial patterns (gradual, immediate, fadeout, anticipation). Empirical Bayes achieves the lowest level MSE under no effect (0.89 vs.\ Kalman's 4.38) because EB shrinks toward zero, which is the truth under this pattern. But EB's advantage reverses under treatment: its level MSE is 3--4$\times$ Kalman's for gradual and anticipation, because shrinking toward zero is the wrong target when effects are nonzero.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../figs/paper_fig2.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../figs/paper_fig3.pdf}
...
\subsection{Sensitivity to Process Noise \texorpdfstring{$\mathbf{Q}$}{Q}}\label{sec:sensitivity}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../figs/paper_fig4.pdf}
...
\paragraph{Formal specification tests.} Three standard tests formalize the visual diagnostics. The Ljung-Box test detects autocorrelation in standardized innovations; rejection suggests $\mathbf{Q}$ misspecification. The Jarque-Bera test detects non-normality; heavy tails indicate that robust Kalman variants (replacing squared loss with Huber loss) may improve robustness. A CUSUM test on cumulative innovations detects structural instability; sustained departures suggest the smoothness assumption is violated by discrete jumps. With $T \approx 24$ periods, power for these tests is limited, so we recommend them as supplements to visual inspection rather than formal gatekeepers.
...
\end{equation}
can be maximized over $\mathbf{Q}$ to select the process noise automatically, eliminating the tuning parameter. This is the approach taken in state-space model estimation \citep{durbin2012time}. We recommend selecting $\mathbf{Q}$ via marginal likelihood maximization rather than relying on defaults. The defaults in Section~\ref{sec:sensitivity} are calibrated starting points, but data-driven selection via MLE is preferable when computationally feasible.
...
\paragraph{Robustness to adversarial DGPs.} The simulation DGPs in Table~\ref{tab:mse} include several smooth trajectories where the local linear trend prior is well-matched. To assess robustness, we report MSE for adversarial patterns: a sawtooth (high-frequency oscillation alternating between 0 and 0.3) and multiple discrete steps (effects of 0.2, 0.4, 0.1 in successive intervals). For the sawtooth, the Kalman smoother achieves only modest MSE reduction relative to raw estimates because the smoother cannot track the high-frequency oscillation. For the multi-step pattern, gains are intermediate. These results reflect the bias-variance tradeoff inherent in any smoothing approach: the smoother sacrifices ability to track high-frequency variation in exchange for variance reduction. When researchers expect high-frequency treatment effect dynamics, they should increase process noise $\mathbf{Q}$ or use raw estimates. See Table~4 in the supplementary materials.

\paragraph{Independence across periods.} Assumption~\ref{a:normal} requires independence of $\hat{\beta}_t$ across $t$. In practice, TWFE estimates share fixed effects and a common residual variance estimate, inducing non-trivial off-diagonal entries in $\text{Var}(\hat{\boldsymbol\beta})$. Our baseline implementation treats $R_t$ as diagonal (scalar $\hat{\sigma}_t^2$), which is exact when the panel is large and errors are independent across units and time. When this fails---particularly with persistent errors or staggered adoption---the diagonal approximation introduces bias.

Simulations with AR(1) correlated observation noise show that the diagonal approximation remains reasonably accurate even with moderate correlations: at $\rho = 0.3$, MSE increases by approximately 57\% relative to the uncorrelated case (7.67 vs.\ 4.88 for the gradual pattern), but the Kalman smoother still substantially outperforms raw estimates. The primary impact of correlation is to increase overall MSE (because the effective sample size is reduced), not to introduce systematic bias in the smoothed estimates. Practitioners should extract the full VCV from their regression and check whether $\max_{s \neq t}|\text{corr}(\hat{\beta}_s, \hat{\beta}_t)| > 0.1$; if correlations are substantial, they should be aware that MSE gains may be smaller than reported in Table~\ref{tab:mse}.

\paragraph{Heavy-tailed errors.} If the Q-Q diagnostic reveals fat tails or the Jarque-Bera test rejects normality, the Kalman smoother may give excessive weight to outlying $\hat{\beta}_t$. Robust extensions replacing squared loss with Huber or $L_1$ loss are available \citep[see][Chapter 9]{durbin2012time} but rarely necessary for event study applications where $\hat{\beta}_t$ derives from regression on many clusters.
...
\paragraph{Pre-testing concerns.} \citet{roth2022pretest} shows that conditioning on passing a pre-test distorts inference. Our test detects slightly more violations, though the power gains are modest. However, violations that survive our test are precisely those the smoother has shrunk the most, raising questions about the interaction between power gains and conditioning bias that warrant further investigation. Researchers concerned about conditioning bias should consider sensitivity analysis \citep{rambachan2023more} rather than relying solely on pre-tests, regardless of which test statistic is used.
...
\paragraph{Why not full Bayesian inference?} A natural extension would specify priors on $\mathbf{Q}$ and perform full posterior inference via MCMC. The Kalman filter provides a practical middle ground: closed-form solutions, computational simplicity, and no prior elicitation beyond the state-space structure. The frequentist bootstrap provides valid inference without requiring researchers to defend Bayesian prior choices.

\paragraph{Derivative interpretation.} The derivative estimates $\widehat{\Delta\beta}^{\rm KS}_t$ inherit more from the smoothness assumption than the level estimates. With noisy observations, there is limited direct information about $\Delta\beta_t$---the prior plays a larger role. Researchers should interpret $\widehat{\Delta\beta}^{\rm KS}_t$ as ``the derivative under the smoothness assumption'' rather than a model-free estimate. The 98\% MSE reduction reflects that raw first differences are dominated by noise, so even a weak prior helps substantially.

\paragraph{Replication materials.} Code and replication materials are available at \url{https://github.com/finite-sample/kalman-event}.
